...
;_______________________________________________________________________________
BUZZER_ON MACRO
        BSF     PORTC,  RC0     ; Env├¡a 5V al buzzer
ENDM
;-------------------------------------------------------------------------------
BUZZER_OFF MACRO
        BCF     PORTC,  RC0     ; Corta la se├▒al al buzzer
ENDM
;_______________________________________________________________________________
;
CFG_SECUENCES MACRO
...
MAIN_LOOP
        ...
        BTFSC   SW1             ; El pin RE0 est├í en presionado (en 0)?
        GOTO    MAIN_LOOP       ; NO -> El pin lee 1, se repite TEST_LEDS
        CALL    SECUENCES       ; SI -> Entra a la secuencias de alarma ac├║stica y visual
...
LOOP_RL
        CALL    FORWARD_LED
        DECFSZ  COUNTER_SECUENCES, F ; Resta 1 al contador y eval├║a si lleg├│ a 0
        GOTO    LOOP_RL              ; NO -> se repite el barrido
        CFG_SECUENCES                ; SI -> el contador llego a 0, es decir, se repiti├│ 3 veces
RETURN
;
;*******************************************************************************
...
FW_LOOP
       LEDS_RLF                 ; Desplaza el led encendido una posici├│n a la derecha (LED0->LED1...)
       CALL     DELAY_3LOOP
       BTFSS LED7               ; LED7 = ON?
...
;
; @details  Desplaza un ├║nico LED encendido desde la posici├│n actual hacia
;           la derecha (hacia LED0) utilizando la instrucci├│n RRF. Finaliza
...
BW_LOOP
        ...
        BTFSS   PORTD,  0       ; LED0 = ON? (Verifica si el '1' lleg├│ al final)
...
;
; @details  Enciende los LEDs secuencialmente ingresando unos l├│gicos desde
...
;
; @details  Apaga los LEDs secuencialmente ingrsando ceros l├│gicos desde LED0
;           hacia LED7
;*******************************************************************************
;
PROGRESSIVE_LED_OFF
PROG_OFF_LOOP
        ...
        LEDS_RLF
        CALL    DELAY_3LOOP     ; Espera para visualizar el salto
        MOVF    PORTD,  F       ; Se mueve PORTD sobre s├¡ mismo para actualizar el FLAG Z
        BTFSS   STATUS, Z       ; El puerto D qued├│ en 0?
